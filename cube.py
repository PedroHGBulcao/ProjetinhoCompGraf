...
class SpinningCube:
   ...
   def generate_cube(self):
       ...
       for face, normal in zip(faces, face_normals):
           ...
           if np.dot(rotated_normal, view_dir) <= 0.18:
               continue
   ...
   def project_image(self, src, H, dst_size):
       ...
       if np.any(dst_x == 0) or np.any(dst_x == w_out - 1) or np.any(dst_y == 0) or np.any(dst_y == h_out - 1):
   ...
   def project(self, point, width, height, scale=550):
       x, y, z = point
       z += 5
       f = scale / z
